...
export function CleanerCard({ cleaner }: CleanerCardProps) {
  return (
    <Card className="group overflow-hidden border-border/50 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300" data-testid={`card-cleaner-${cleaner.id}`}>
      ...
      <CardHeader className="p-5 pb-2">
        <div className="flex justify-between items-start">
          <h3 className="text-xl font-bold line-clamp-1">{cleaner.name}</h3>
        </div>
      ...
      <CardFooter className="p-5 pt-0 mt-auto">
        <Link href={`/cleaners/${cleaner.id}`} className="w-full">
          <Button className="w-full gap-2" data-testid={`button-view-profile-${cleaner.id}`}>
            View Profile
          </Button>
        </Link>
      </CardFooter>
